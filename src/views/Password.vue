...
const validatePass = async () => {
	...
	try {
		...
		if (user.role === 'CUISINE') {
			...
			error.value = "Ce compte n'a pas accès à la cuisine"
		}
		loading.value = false
	} catch (err) {
...
<template>
	<div class="min-h-screen flex flex-col justify-center items-center p-8">
		...
		<div class="w-full max-w-[460px] rounded-xl bg-white border border-black/[.07] shadow-sm overflow-hidden">
			<div class="flex items-center gap-3 px-5 h-[76px] border-b border-border">
				<span class="w-12 h-12 rounded-lg bg-main/[.10] flex items-center justify-center">
					<Kitchen class="h-7 fill-main" />
				</span>
				<div>
					<h1 class="font-bree-serif text-xl leading-tight">Cuisine</h1>
