...
<template>
  <div class="bg-white rounded-md p-2 flex flex-col gap-1 cursor-pointer">
    <div class="flex gap-2 items-center">
      <h3 class="text-lg">La commande N&#186 {{ item.number }}</h3>
      <span class="text-black/60 text-base">({{ setType(item.type) }})</span>
    </div>
    <div v-if="showDetails" class="flex flex-col gap-4 py-4">
      ...
      >
        <h2 class="text-black/80 text-sm font-bree-serif">{{ product.name }} <span v-if="product.size" class="uppercase text-main">({{ product.size }})</span></h2>
        <p class="flex flex-col gap-1 text-black/40 text-xs">
